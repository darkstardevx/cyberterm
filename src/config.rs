...
pub struct CyberConfig {
    ...
    // --- CLI Structural Theme Fields ---
    pub theme_category: String,
    pub theme_folder: String,
...
impl Default for CyberConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            theme_folder: "core".to_string(),
            theme_variant: "synthwave_84".to_string(),
            opacity: 0.90,
            wallpaper: None,
...
fn initialize_builtin_themes(themes_dir: &std::path::Path) -> Result<(), std::io::Error> {
    ...
    for (filename, content) in themes {
        let theme_path = themes_dir.join(format!("{}.theme", filename));
